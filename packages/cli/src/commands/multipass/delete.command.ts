...
export class DeleteCommand extends BaseCommand {
  ...
  private constructor(deps: DeleteCommandDeps = {}) {
    super();
    this.resolveInstances =
      deps.resolve ?? MultipassConfigReader.resolve.bind(MultipassConfigReader);
  }

  static create(deps?: DeleteCommandDeps): Command {
    return new DeleteCommand(deps).build();
  }

  build(): Command {
    return new Command('delete')
      .description('Delete and purge Multipass VMs')
      .argument('[names...]', 'VM names to delete')
      .option('--all', 'Delete all instances defined in config')
  ...
  private async execute(
    ...
    for (const instance of instances) {
      console.log(chalk.cyan(`Deleting ${instance.name}...`));
      const success = await this.runMultipass([
        'delete',
        instance.name,
        '--purge',
      ]);
      if (!success) {
        ...
        console.error(chalk.red(`✖ Failed to delete ${instance.name}`));
      } else {
        console.log(chalk.green(`✔ Deleted ${instance.name}`));
      }
    }

    if (failures.length > 0) {
      this.fail(`Delete failed for: ${failures.join(', ')}`);
    }
  }
...
export const deleteCommand = DeleteCommand.create();
