...
const { status, data } = await apiInstance.detachVolume(id);
```
...
const { status, data } = await apiInstance.getVolumeAction(id, actionId);
```
...
const { status, data } = await apiInstance.getVolumesAction(id);
```
