...
const { status, data } = await apiInstance.getCertificateAction(
    id,
    actionId
);
```
...
const { status, data } = await apiInstance.getCertificatesAction(
    id
);
```
...
const { status, data } = await apiInstance.retryCertificate(
    id
);
```
