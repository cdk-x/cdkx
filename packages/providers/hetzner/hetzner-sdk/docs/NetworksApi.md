...
const { status, data } = await apiInstance.createNetwork(networkCreateRequest);
```
...
const { status, data } = await apiInstance.deleteNetwork(id);
```
...
const { status, data } = await apiInstance.getNetwork(id);
```
