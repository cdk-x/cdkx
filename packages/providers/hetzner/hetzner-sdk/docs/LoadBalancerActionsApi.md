...
const { status, data } =
  await apiInstance.disableLoadBalancerPublicInterface(id);
```
...
const { status, data } =
  await apiInstance.enableLoadBalancerPublicInterface(id);
```
...
const { status, data } = await apiInstance.getLoadBalancerAction(id, actionId);
```
...
const { status, data } = await apiInstance.getLoadBalancersAction(id);
```
