...
const { status, data } = await apiInstance.deleteImage(id);
```
...
const { status, data } = await apiInstance.getImage(id);
```
...
const { status, data } = await apiInstance.updateImage(id, updateImageRequest);
```
