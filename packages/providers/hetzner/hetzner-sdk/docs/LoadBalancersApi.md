...
const { status, data } = await apiInstance.deleteLoadBalancer(id);
```
...
const { status, data } = await apiInstance.getLoadBalancer(id);
```
...
let type: Array<
  ...
  | 'requests_per_second'
  | 'bandwidth'
>; //Type of metrics to get. (default to undefined)
