...
const { status, data } = await apiInstance.getPrimaryIpAction(
    id,
    actionId
);
```
...
const { status, data } = await apiInstance.getPrimaryIpsAction(
    id
);
```
...
const { status, data } = await apiInstance.unassignPrimaryIp(
    id
);
```
