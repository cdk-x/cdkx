...
const { status, data } = await apiInstance.getFloatingIpAction(id, actionId);
```
...
const { status, data } = await apiInstance.getFloatingIpsAction(id);
```
...
const { status, data } = await apiInstance.unassignFloatingIp(id);
```
