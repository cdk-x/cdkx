...
const { status, data } = await apiInstance.addNetworkRoute(
    id,
    addRouteRequest
);
```
...
const { status, data } = await apiInstance.getNetworkAction(
    id,
    actionId
);
```
...
const { status, data } = await apiInstance.getNetworksAction(
    id
);
```
