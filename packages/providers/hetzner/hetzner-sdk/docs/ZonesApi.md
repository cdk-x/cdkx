...
const { status, data } = await apiInstance.createZone(createZoneRequest);
```
...
const { status, data } = await apiInstance.deleteZone(idOrName);
```
...
const { status, data } = await apiInstance.getZone(idOrName);
```
...
const { status, data } = await apiInstance.getZoneZonefile(idOrName);
```
