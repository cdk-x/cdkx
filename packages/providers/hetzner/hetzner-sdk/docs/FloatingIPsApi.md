...
const { status, data } = await apiInstance.deleteFloatingIp(
    id
);
```
...
const { status, data } = await apiInstance.getFloatingIp(
    id
);
```
...
let sort: Array<'id' | 'id:asc' | 'id:desc' | 'created' | 'created:asc' | 'created:desc'>; //Sort resources by field and direction. Can be used multiple times. For more information, see \"[Sorting](#description/sorting)\".  (optional) (default to undefined)
