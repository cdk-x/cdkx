...
10. **Create `AI.md`** — add an `AI.md` at the package root
    documenting:
...
11. **Add to `opencode.json` instructions** — add the new package's
    `packages/<name>/AI.md` to the `instructions` array in `opencode.json`
...
- **Conventional Commits**: For detailed commit conventions, use the `cdkx-conventional-commits` skill.
- **Clean Code**: Use the `clean-code-principles` skill for SOLID principles and best practices.
- **TypeScript**: Use the `typescript-best-practices` skill for TypeScript-specific guidelines.
